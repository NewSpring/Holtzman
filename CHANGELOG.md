...
* Turned linting back on.
* Turned storybook back on as well.
* Added Group Member Status to the group query and will only show a group member if they are active.

### Fixed

* All the linting errors that had accumulated since we turned linting off.
* A bug that was stopping the forgot password emails from being sent.
